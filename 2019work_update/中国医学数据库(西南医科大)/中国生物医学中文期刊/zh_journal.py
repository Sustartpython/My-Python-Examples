...
def journal_list():
    url = 'http://www.sinomed.ac.cn/zh/journalSearch.do?method=py'
    ...
    for root, dirs, files in os.walk(journal_list_path):
        ...
        for file in files:
            ...
            for i,item in enumerate(journal_url_list):
                journal_url = "http://www.sinomed.ac.cn" + item
                j_id = re.findall("detail&id=(.*)&db=journal",item)[0]
                journal_name = html.xpath("//table[@class='tabel-lab']//tr/td/a/text()").extract()[i].replace("\\",'-').replace("/","-").replace(":","-").replace("*","-").replace("?","-").replace("\"","-").replace("<","-").replace(">","-").replace("|","-")
                journal_place = html.xpath("//table[@class='tabel-lab']//tr/td[2]/span/text()").extract()[i]
                result.append(
            ...
            for j_id, journal_url, journal_name in rows:
                try:
                    ...
                    if res.text.find(feature) < 0:
                        ...
                        # 获取期刊年份 post请求
                        year_url = 'http://www.sinomed.ac.cn/zh/journalSearch.do?method=hidden'
                        data = {
                            'dbtype': 'zh',
                            'db': 'journal',
                ...
                if 'journalNameNull' in cover_url:
                    ...
                    try:
                        ...
                        if res.status_code == 200:
                            ...
                            provider = 'sinomedzhjournal'
                            pathtxt = r'E:\down_data_e\sinomed\zh\sinomedzhjournal_cover_20191028.txt'
...
def get_article_list():
    ...
    url = "http://www.sinomed.ac.cn/zh/basicSearch.do"
    result = []
    while True:
        conn = pymysql.connect(DBHOST, DBUSER, DBPWD, DB)
        cur = conn.cursor()
            ...
            for j_id, journal_name, journal_id, year in rows:
                ...
                try:
                    ...
                    if res.text.find(feeee) < 0:
                        ...
                        page_num = math.ceil(20 * int(page_num_20) / 500)
                        ...
                        while True:
                            if not article_list_que.empty():
                                ...
                                file_name = j_id + '_' + journal_name
                                shijian = '20191028'
                                file_path = article_list_2_path + '\\' + shijian + '\\' + file_name + '\\' + str(year)
                                ...
                                if os.path.exists(fname):
                                    ...
                                    try:
                                        res = requests.post(url,data=data,proxies=Proxy,headers=Header)
                                        res.encoding = res.apparent_encoding
                                        feeee = 'right-wztxt fL'
                                        if res.text.find(feeee) < 0:
                                            # with open("1.html",'w',encoding='utf-8')as f:
                                            ...
                                            print("%s-%s_%s--%s--page-%s down right" % (times, journal_name, j_id, year, str(page)))
                                            result.append(
                                                (journal_name,year)
                                            )
                                    except Exception as e:
...
def parse_article_list():
    ...
    result = []
    for root, dirs, files in os.walk(article_list_2_path):
        conn = pymysql.connect(DBHOST, DBUSER, DBPWD, DB)
        cur = conn.cursor()
        for file in files:
            file_path = root + '/' + file
            print(file_path)
            id_name = re.findall(r'E:\\down_data_e\\sinomed\\zh\\article_list_2\\20191028\\(.*)\\',file_path)[0]
...
def uphdfs():
    ...
    HdfsDir = r'/RawData/SinoMed/zh/big_json/%s/%s'%(year,now_time)
...
def raname_pp():
    ...
    article_list_path = r'E:\down_data_e\sinomed\zh\article_list'
    ...
    for file_name in files:
        ...
        # print(file_name)
        journal_name = file_name
        # conn = pymysql.connect(DBHOST, DBUSER, DBPWD, DB)
        # cur = conn.cursor()
        sql = "select j_id from journal_list where journal_name = %s"
        ...
        if len(rows) == 0:
            ...
            j_id = rows[0][0]
            print("%s----%s"%(journal_name,rows[0][0]))
