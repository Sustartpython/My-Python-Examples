...
class CambridgeJournal(Provider):
	...
	def update(self):
		# self.initpath()	# self.upload2HDFS(None)		
	...
	def startdown_list(self, message):
		...
		if self.totalcount == 0:
			if len(os.listdir(self.list_path)) == 0:
				...
				return
		flag = self.refreshsession()
		print(flag)
		# print('xiazai'+len(rows))
		for url, _ in rows:
	...
	def startdown_cover(self, message):
		...
		for url, cover_url in rows:
			...
			self.sendwork('down_cover', (url, filename,0))
