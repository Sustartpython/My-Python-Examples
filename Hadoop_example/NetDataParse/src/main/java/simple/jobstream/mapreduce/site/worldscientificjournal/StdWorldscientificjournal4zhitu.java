...
//输入应该为去重后的html
public class StdWorldscientificjournal4zhitu extends InHdfsOutHdfsJobInfo {
	...
	public void pre(Job job) {
		String jobName = "worldscientificjournal." + this.getClass().getSimpleName();
...
//			String page = "";
			...
			for (Map.Entry<String, String> updateItem : xObj.data.entrySet()) {
				if (updateItem.getKey().equals("lngid")) {
					...
					if(updateItem.getValue().trim().indexOf("pdf") != -1) {
						if_pdf_fulltext = "1";
					}else {
						if_pdf_fulltext = "0";
					}
			...
			String provider = "worldscientificjournal";
			...
			String sql = "insert into modify_title_info_zt("
					+ "lngid,rawid,batch,title,identifier_pissn,"
					+ "identifier_eissn,identifier_doi,creator,creator_institution,source,"
					+ "publisher,date,volume,issue,subject," 
					+ "description,date_created,ref_cnt,cited_cnt,down_cnt,"
					+ "rawtype,if_pdf_fulltext,is_oa,medium,provider,"
					+ "provider_id,provider_url,provider_subject,type,country,language)";
			sql += " VALUES ('%s','%s','%s','%s','%s',"
					...
					+ "'%s','%s','%s','%s','%s',"
					+ "'%s','%s','%s','%s','%s','%s');";
			sql = String.format(sql,lngid,rawid,batch,title,identifier_pissn,
					identifier_pissn,identifier_doi,creator,creator_institution,source,
					publisher,date,volume,issue,subject,
					description,date_created,ref_cnt,cited_cnt,down_cnt,
					rawtype,if_pdf_fulltext,is_oa,medium,provider,
					provider_id,provider_url,provider_subject,type,country,language);
