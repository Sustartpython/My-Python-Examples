...
//输入应该为去重后的html
public class Std2DbZTCnipr extends InHdfsOutHdfsJobInfo {
	private static Logger logger = Logger.getLogger(Std2DbZTCnipr.class);
	...
	private static String postfixDb3 = "cniprpatent";
	//private static String tempFileDb3 = "/RawData/_rel_file/zt_template.db3";
	//上传更新字段时使用
//	private static String tempFileDb3 = "/user/xujiang/zt_template_update.db3";
	...
	public static class ProcessMapper extends Mapper<Text, BytesWritable, Text, NullWritable> {
		...
		private static void initPubNoSet(Context context) throws IOException {
			...
			FSDataInputStream fin = fs.open(new Path("/RawData/cnipapatent/big_cover/20181029/txtcover.txt"));
...
//			String lngid = "CNIPA_ZL_" + rawid;
			...
			String date = pub_date.substring(0, 4);
...
//				cover = "/smartlib/cnipapatent/"+cndir+"/"+cndir2+"/"+pub_no.toLowerCase()+".jpg";
//			}
...
//			}
			...
			String sql = "INSERT INTO modify_title_info_zt([lngid]," + "[rawid]," + "[title],"
					+ "[identifier_standard]," + "[date_impl]," + "[identifier_pissn]," + "[date]," + "[date_created],"
					+ "[applicant]," + "[creator]," + "[creator_institution]," + "[subject_csc],"
					+ "[subject_isc]," + "[agency]," + "[agents]," + "[description]," + "[country]," + "[language],"
					+ "[rawid]," + "[provider_url]," + "[provider_id]," + "[provider]," + "[medium]," + "[type],"
					+ "[batch]," + "[pct_enter_nation_date]," + "[pct_app_data]," + "[pct_pub_data],"
					+ "[priority_number]," + "[cover])";
			sql += " VALUES ('%s', '%s', '%s', '%s', '%s','%s','%s','%s','%s','%s',"
					+ "'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',"
					+ "'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');";
			sql = String.format(sql, lngid, rawid, title, pub_no, app_date, app_no, date, pub_date, applicant, author,
					applicant_addr, ipc_no_1st, ipc_no, agency, agent, abstracts, country, language, rawid,
					provider_url, provider_id, provider, medium, type_, batch, pct_enter_nation_date, pct_app_data,
					pct_pub_data, priority, cover_path);


			sql = "INSERT INTO modify_title_info_zt([lngid],[batch],[provider],[date],[type],[medium],[cover])";
			sql += " VALUES ('%s','%s','%s','%s','%s','%s','%s');";
			sql = String.format(sql, lngid, batch,provider,date,type_,medium,cover_path); 
