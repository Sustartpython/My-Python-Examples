...
public class sipoCover2XXXXObject extends InHdfsOutHdfsJobInfo {
	...
	public static class ProcessMapper extends 
	    ...
	    public void map(LongWritable key, Text value, Context context
			...
			String requestid = "";
			String cover_path = "";
			requestid = mapField.get("requestid").toString().trim();
			cover_path = mapField.get("cover").toString().trim();
			
			if ("".equals(requestid) || "".equals(cover_path) ) {
				context.getCounter("map", "covernull").increment(1);
			}
            
            XXXXObject xObj = new XXXXObject();
			xObj.data.put("rawid", requestid);
			xObj.data.put("cover_path", cover_path);
			context.getCounter("map", "count").increment(1);
			
			byte[] bytes = VipcloudUtil.SerializeObject(xObj);
			context.write(new Text(requestid), new BytesWritable(bytes));			
	    }				
	}
