...
public class Temp2Latest4Clean extends InHdfsOutHdfsJobInfo {
	...
	public static String inputHdfsPath =  "/RawData/wanfang/qk/detail/latest_bak20190603";
	public static String outputHdfsPath = "/RawData/wanfang/qk/detail/latest";
		...
		public void map(Text key, BytesWritable value, Context context) throws IOException, InterruptedException {
			...
			{// cited_cnt
				...
				if (cited_cnt.length() < 1) {
					...
					context.getCounter("map", "cited_cnt@20190101").increment(1);
				}
			}
			
			{// down_cnt
				if (down_cnt.indexOf('@') < 0) {
					boolean isMatch = Pattern.matches("^\\d+$", down_cnt);
					if (isMatch) {
						down_cnt = down_cnt + "@20190101";
					}
					else {
						down_cnt = "";
					}
				...
				if (down_cnt.length() < 1) {
					context.getCounter("map", "down_cnt blank").increment(1);
				}
				else if (down_cnt.endsWith("@20190101")) {
					context.getCounter("map", "down_cnt@20190101").increment(1);
				}
			}			

			xxxobj.data.put("cited_cnt", cited_cnt);
			xxxobj.data.put("down_cnt", down_cnt);
