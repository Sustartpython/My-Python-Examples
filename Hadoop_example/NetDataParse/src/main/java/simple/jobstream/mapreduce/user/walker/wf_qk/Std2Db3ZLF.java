...
//		JobConfUtil.setTaskPerReduceMemory(job, 5120);
	...
	public static class ProcessMapper extends Mapper<Text, BytesWritable, Text, NullWritable> {
		...
		private static String getBookId(String pykm, String years, String num) {
			...
			bookid = VipIdEncode.getLngid("00004", line, false);
		...
		public void map(Text key, BytesWritable value, Context context) throws IOException, InterruptedException {
			// 测试
//			if (!key.toString().toLowerCase().startsWith("gwyx-lcfsxfc2019")) {
//				return;
//			}
			...
			String sql = "INSERT INTO main([lngid], [bookid], [issn], [cnno], [rawid], [pub1st], "
					...
					+ "[doi], [class], [auto_class],[firstclass], [years], [vol], [num], [fulltext_type],"
					+ "[fromtype], [down_date]) ";
					...
					+ "'%s', '%s', '%s');";
			{
				...
				String author_e = author_alt.replace('\0', ' ').replace("'", "''").trim();
				...
				String auto_class = clc_machine.replace('\0', ' ').replace("'", "''").trim();
...
//				lngid = getLngIDByWanID(rawid);
				lngid = VipIdEncode.getLngid("00045", rawid, false);
				String pykm = journal_raw_id;
				String bookid = getBookId(pykm, years, num);
				String pub1st = "0";
				String fromtype = "WANFANG";
				sql = String.format(sql, lngid, bookid, issn, cnno, rawid, pub1st, muinfo, pageline, pagecount, ref_cnt,
						...
						keyword_c, keyword_e, imburse, doi, sClass, auto_class, firstclass, years, vol, num, fulltext_type,
						fromtype, down_date);
			}
