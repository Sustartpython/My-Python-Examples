...
public class GenNewData extends InHdfsOutHdfsJobInfo {
	...
	{
		String jobName = "cnkithesis.GenNewData";
	...
	public static class ProcessMapper extends 
	    ...
	    public void map(Text key, BytesWritable value, Context context
			...
			if (inputPath.equals("/bs/latest_temp")) {		
				context.getCounter("map", "旧数据").increment(1);
				xObj.data.put("旧数据", "false");	//标识应该保留的数据
			}
			else {
				context.getCounter("map", "新数据").increment(1);
				xObj.data.put("新数据", "true");	//本趟新数据
			}
	    ...
	    public void reduce(Text key, Iterable<BytesWritable> values, 
			...
			for (BytesWritable item : values) {
				...
				for (Map.Entry<String, String> updateItem : xObj.data.entrySet()) {
					if (updateItem.getKey().equals("新数据")) {
