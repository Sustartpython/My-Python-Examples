...
// 将A层数据导出成智立方db3格式
public class Std2Db3ZLF extends InHdfsOutHdfsJobInfo {
	private static Logger logger = Logger.getLogger(Std2Db3ZLF.class);
...
//		JobConfUtil.setTaskPerReduceMemory(job, 5120);
	...
	public static class ProcessMapper extends Mapper<Text, BytesWritable, Text, NullWritable> {
		...
		public void map(Text key, BytesWritable value, Context context) throws IOException, InterruptedException {
			...
			// 转义及生成SQL语句
			String sql = "INSERT INTO main([has_oversea], [lngid], [bookid], [issn], [cnno], [rawid], [cnkiid], [pub1st], "
					...
					+ "[remark_c], [remarktype_c], [remark_e], [keyword_c], [keyword_e], [imburse], "
					...
					+ "'%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');";
			{
				...
				String remarktype_c = abstract_type.replace('\0', ' ').replace("'", "''").trim();
				...
				sql = String.format(sql, has_oversea, lngid, bookid, issn, cnno, rawid, rawid, pub1st, muinfo, pageline,
						pagecount, ref_cnt, cited_cnt, beginpage, endpage, jumppage, pykm, title_c, title_e,
						firstwriter, showwriter, cbmwriter, writer, author_e, firstorgan, organ, name_c, name_e,
						showorgan, remark_c, remarktype_c, remark_e, keyword_c, keyword_e, imburse, doi, sClass, firstclass, years,
						vol, num, fromtype, down_date, down_cnt, fulltext_type, if_html_fulltext, publishdate);
			}
