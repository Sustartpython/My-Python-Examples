...
//统计ei的数据量
public class CountEI extends InHdfsOutHdfsJobInfo {
	private static boolean testRun = false;
	private static int testReduceNum = 10;

	private static int reduceNum = 0;
	public static final String inputHdfsPath = "/RawData/EI/latest";
	public static final String outputHdfsPath = "/vipuser/walker/output/CountEI";
	

	
	public void pre(Job job) {
		String jobName = "CountEI";
		if (testRun) {
	...
	public static class ProcessMapper extends
		...
		public void map(Text key, BytesWritable value, Context context)
			...
			for (Map.Entry<String, String> updateItem : xObj.data.entrySet()) {
				...
				else if (updateItem.getKey().equals("PublicationYear")) {
					PublicationYear = updateItem.getValue().trim();
				}
			}
			
			context.getCounter("map", "year-" + PublicationYear).increment(1);			
			context.getCounter("map", "count").increment(1);
			
			context.write(new Text(AccessionNumber), NullWritable.get());
		}
