...
//输入应该为去重后的html
public class StdPlosjournal4zhitu extends InHdfsOutHdfsJobInfo {
...
//			String endpage = ""; 
			...
			String sql = "insert into modify_title_info_zt("
					+ "lngid,rawid,batch,title,title_series,identifier_pissn,"
					+ "identifier_doi,creator,creator_institution,creator_bio,source,"
					+ "publisher,date,volume,issue,description," 
					+ "description_fund,subject,date_created,if_pdf_fulltext,ref_cnt,rawtype,"
					+ "is_oa,medium,provider,provider_id,provider_url,"
					+ "gch,type,country,language)";
			sql += " VALUES ('%s','%s','%s','%s','%s','%s',"
					...
					+ "'%s','%s','%s','%s','%s',"
					+ "'%s','%s','%s','%s');";
			sql = String.format(sql,lngid,rawid,batch,title,title_series,identifier_pissn,
					identifier_doi,creator,creator_institution,creator_bio,source,
					publisher,date,volume,issue,description,
					description_fund,subject,date_created,if_pdf_fulltext,ref_cnt,rawtype,
					is_oa,medium,provider,provider_id,provider_url,
