...
import org.apache.hadoop.yarn.webapp.hamlet.Hamlet.TITLE;
...
public class Std2Db3 extends InHdfsOutHdfsJobInfo {
	...
	private static String postfixDb3 = "jstorjournal";
	...
	public static class ProcessMapper extends
		...
		public void map(Text key, BytesWritable value, Context context)
			...
			for (Map.Entry<String, String> updateItem : xObj.data.entrySet()) {
				...
				}else if(updateItem.getKey().equals("contentIssue")) {
					contentIssue = updateItem.getValue().trim();
				}
			}
			String lngid = "JSTOR_WK_" + id;
			String provider="jstorjournal";
			String type="3";
			String medium="2";
		    gch = provider + "@" + gch;
			...
			creator = "";
			for (String creators:creatorarray) {
				creator += creators.trim()+";";
			}
			if (!creator.equals("")) {
			...
			String sql = "INSERT INTO modify_title_info_zt([lngid], [title],[creator],[source],[publisher],[volume],[issue],[page],[date_created],[date],[language],[identifier_pissn],[identifier_eissn],[identifier_doi],[type],[description],[provider],[batch],[medium],[country],[provider_url],[provider_id],[rawid],[subject],[beginpage],[endpage],[pageCount],[provider_subject],[page],[gch]) ";
			sql += " VALUES ('%s', '%s', '%s', '%s', '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');";
			sql = String.format(sql, lngid, title, creator,source,publisher,volume,issue,page,date_created,date,language,identifier_pissn,identifier_eissn,identifier_doi,type,description,provider,batch,medium,country,provider_url,provider_id, rawid,subject,beginpage,endpagepage,pageCount,provider_subject,page,gch);								
	
//			String sql = "INSERT INTO modify_title_info_zt([rawid],[title],[volume],[issue]) ";
//			sql += " VALUES ('%s','%s','%s','%s');";
//			sql = String.format(sql, rawid,contentIssue,volume,issue);								
