...
// 以A层标准导出数据到db3
public class Std2Db3A_new extends InHdfsOutHdfsJobInfo {
	...
	public static class ProcessMapper extends Mapper<Text, BytesWritable, Text, NullWritable> {
		...
		private static Logger logger = Logger.getLogger(Std2Db3A_new.class);
		...
		public void map(Text key, BytesWritable value, Context context) throws IOException, InterruptedException {
			...
			for (Map.Entry<String, String> item : xObj.data.entrySet()) {
				if (item.getKey().equals("rawid")) {
					rawid = item.getValue().trim();
				}
			}
			for (Map.Entry<String, String> item : xObj.data.entrySet()) {
				...
				String val = "";
				if (ikey.equals("lngid")) {
					 val = VipIdEncode.getLngid("00114", rawid, false);
				}else if(ikey.equals("product")){
					 val = "RDFYBK";
				}else if(ikey.equals("provider_url")){
					val = "http://www.rdfybk.com/qk/pdfview?id="+rawid;
				}else if (ikey.equals("sub_db_id")) {
					 val = "00114";
				}else{
					 val = item.getValue();
				}
