...
public class CountHdfsData extends InHdfsOutHdfsJobInfo {
	...
	public static final String outputHdfsPath = "/vipuser/walker/output/QK/count_hdfs_data";
		...
		public void map(Text key, BytesWritable values, Context context)
			...
			for (Map.Entry<String, String> updateItem : xxxobj.data.entrySet()) {
				if (updateItem.getKey().equals("type")) {
					...
					refertext = updateItem.getValue();
					refertext = refertext.trim();
				}
			}

			/*
			if (language.equals("1") && type.equals("7")) {
				context.getCounter("map", "duxiu ts").increment(1);
				if (lngid.toUpperCase().startsWith("TS")) {
					context.getCounter("map", "duxiu ts TS").increment(1);
				}
			}
			*/
			...
			if (type.equals("99")) {
				...
				context.getCounter("map", "type not 99").increment(1);
			}
			
			if (refertext.length() < 1) {
				context.getCounter("map", "cnki qk").increment(1);
			}
			else {
				context.getCounter("map", "cnki qk ref").increment(1);
			}
