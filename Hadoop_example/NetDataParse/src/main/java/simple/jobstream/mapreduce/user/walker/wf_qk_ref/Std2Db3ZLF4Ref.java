...
//			String lngsourceid = getLngIDByWanID(rawsourceid);
			String lngsourceid = VipIdEncode.getLngid("00045", rawsourceid, false);
			lngid = lngsourceid + lngid.substring(lngid.length()-4);

			String sql = "INSERT INTO ref_zk([id], [rawsourceid], [lngsourceid], [refertext], [strtitle], [strtype], [strname], [strwriter1], [stryearvolnum], [strpubwriter], [strpages],[doi], [disproof_id], [down_date]) ";
			...
			// 转义
			{
				...
				String stryearvolnum = pub_year.replace('\0', ' ').replace("'", "''").trim();
				...
				String disproof_id = linked_id.replace('\0', ' ').replace("'", "''").trim();
				down_date = down_date.replace('\0', ' ').replace("'", "''").trim();
				
				sql = String.format(sql, lngid, rawsourceid, lngsourceid, refertext, strtitle, strtype, strname, strwriter1,
						stryearvolnum, strpubwriter, strpages, doi, disproof_id, down_date);
			}
