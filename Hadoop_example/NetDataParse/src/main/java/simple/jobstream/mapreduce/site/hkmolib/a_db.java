...
// 以A层标准导出数据到db3（期刊）
public class a_db extends InHdfsOutHdfsJobInfo {
		...
		public void map(Text key, BytesWritable value, Context context)
			...
			for (Map.Entry<String, String> updateItem : xObj.data.entrySet()) {
				if (updateItem.getKey().equals("lngid")) {
					...
					degree = updateItem.getValue().trim();
				}else if (updateItem.getKey().equals("degree_apply")) {
					degree_apply = updateItem.getValue().trim();
				}
			}
			...
			//转义
			{
				...
				degree = degree.replace('\0', ' ').replace("'", "''").trim();
				degree_apply = degree_apply.replace('\0', ' ').replace("'", "''").trim();
			}
								   
			String sql = "INSERT INTO base_obj_meta_a([degree],[degree_apply],[lngid], [rawid], [sub_db_id], [product], [sub_db], [provider], [down_date], [batch], [doi], [source_type], [provider_url], [title], [title_alt], [title_sub], [title_series], [keyword], [keyword_alt], [keyword_machine], [clc_no_1st], [clc_no], [clc_machine], [subject_word], [subject_edu], [subject], [abstract], [abstract_alt], [abstract_type], [abstract_alt_type], [page_info], [begin_page], [end_page], [jump_page], [doc_code], [doc_no], [raw_type], [recv_date], [accept_date], [revision_date], [pub_date], [pub_date_alt], [pub_place], [page_cnt], [pdf_size], [fulltext_txt], [fulltext_addr], [fulltext_type], [column_info], [fund], [fund_alt], [fund_id],[author_id], [author_1st], [author], [author_raw], [author_alt], [corr_author], [corr_author_id], [email], [subject_dsa], [research_field], [contributor], [contributor_id], [contributor_alt], [author_intro], [organ_id], [organ_1st], [organ], [organ_alt], [preferred_organ], [host_organ_id], [organ_area], [journal_raw_id], [journal_name], [journal_name_alt], [pub_year], [vol], [num], [is_suppl], [issn], [eissn], [cnno], [publisher], [cover_path], [is_oa], [country], [language], [ref_cnt], [ref_id], [cited_id], [cited_cnt], [down_cnt], [is_topcited], [is_hotpaper]) ";
			...
			sql = String.format(sql, degree,degree_apply,lngid, rawid, sub_db_id, product, sub_db, provider, down_date, batch, doi, source_type, provider_url, title, title_alt, title_sub, title_series, keyword, keyword_alt, keyword_machine, clc_no_1st, clc_no, clc_machine, subject_word, subject_edu, subject, abstract_, abstract_alt, abstract_type, abstract_alt_type, page_info, begin_page, end_page, jump_page, doc_code, doc_no, raw_type, recv_date, accept_date, revision_date, pub_date, pub_date_alt, pub_place, page_cnt, pdf_size, fulltext_txt, fulltext_addr, fulltext_type, column_info, fund, fund_alt, fund_id, author_id, author_1st, author, author_raw, author_alt, corr_author, corr_author_id, email, subject_dsa, research_field, contributor, contributor_id, contributor_alt, author_intro, organ_id, organ_1st, organ, organ_alt, preferred_organ, host_organ_id, organ_area, journal_raw_id, journal_name, journal_name_alt, pub_year, vol, num, is_suppl, issn, eissn, cnno, publisher, cover_path, is_oa, country, language, ref_cnt, ref_id, cited_id, cited_cnt, down_cnt, is_topcited, is_hotpaper);								
