 ...
 */
public class MergeXXXXObjectMergeDateDynamicValue extends InHdfsOutHdfsJobInfo {
	...
	public static class ProcessReducer extends Reducer<Text, BytesWritable, Text, BytesWritable> {
		public void reduce(Text key, Iterable<BytesWritable> values, Context context)
			...
			if (cnt < 2) { // 一个值，直接输出
				...
				// 合并下载量（down_cnt）
				if (xObjMajor.data.containsKey("down_cnt")) {
					String downCntMajor = xObjMajor.data.get("down_cnt").trim();
					String downCntMinor = "";
					if (xObjMinor.data.containsKey("down_cnt")) {
						downCntMinor = xObjMinor.data.get("down_cnt").trim();
					}

					xObjMajor.data.put("down_cnt", DateTimeHelper.mergeDateDynamicValue(99, downCntMajor, downCntMinor));

				}

				// 合并被引量（cited_cnt）
				if (xObjMajor.data.containsKey("cited_cnt")) {
					String citedCntMajor = xObjMajor.data.get("cited_cnt").trim();
					String citedCntMinor = "";
					if (xObjMinor.data.containsKey("cited_cnt")) {
						citedCntMinor = xObjMinor.data.get("cited_cnt").trim();
					}
					xObjMajor.data.put("cited_cnt", DateTimeHelper.mergeDateDynamicValue(99, citedCntMajor, citedCntMinor));
				}
			}
